...
{
    public class SalesOrderDetail_PreCreate:IPlugin
    {
        ...
        {
            //Entity sod = (Entity)context.InputParameters["Target"]; //orgService.Retrieve(context.PrimaryEntityName, context.PrimaryEntityId, new ColumnSet(true));

            //检查是否存在已提交状态
            Entity so = orgService.Retrieve("salesorder", ((EntityReference)((Entity)context.InputParameters["Target"])["salesorderid"]).Id,
        ...
        {
            Entity sod = orgService.Retrieve("salesorderdetail", ((EntityReference)context.InputParameters["Target"]).Id, new ColumnSet(true));
